...
import {
  ...
  addReturnVisitRecordParamsType,
  deleteSoldClues,
  addSoldClues,
  addSoldCluesParamsType,
...
const service: React.FC<ServiceProps> = ({ dispatch, list, total, loading, realname }) => {
  ...
  const columns: ColumnProps<ListItemType>[] = [
    ...
    },
    {
      ...
      align: 'center',
      render: record =>
        record?.custReturnVisitRecords[0]?.returnVisitTime &&
        moment(record.custReturnVisitRecords[0]?.returnVisitTime).format('YYYY-MM-DD HH:mm:ss'),
    },
    {
      title: '最后回访人员',
      align: 'center',
      ...
      render: record => (
        <>
          <Button
            ...
            style={{ width: 40, color: '#1A61DC', padding: 0 }}
          >
            添加
          </Button>
          <Button
            type="link"
            onClick={() => router.push(`/service/manager/salesLeads/edit?id=${record.code}`)}
            style={{ width: 40, color: '#999999', padding: 0 }}
          >
            查看
          </Button>
          <Button
  ...
  /** 删除回访记录 */
  const deleteList = async (cluesCode: any) => {
    Modal.confirm({
      title: '提示',
      content: '是否删除',
      okText: '确定',
  ...
  return (
    <div className={Styles.container}>
      <div className={Styles.toolbar}>
        <div onClick={() => setSouceVisible(true)}>
          <Icon type="plus" />
          添加线索
        </div>
      </div>
