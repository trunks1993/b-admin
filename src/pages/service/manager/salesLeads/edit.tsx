      ...
      <div className={'kehufilter'}>
        <div className={Styles.filter} style={{ paddingTop: 20 }}>
          ...
          <Descriptions bordered={true} column={{ md: 2 }}>
            ...
            <Descriptions.Item label="最后回访时间">
              {detaillist?.custReturnVisitRecords?.[0]?.returnVisitTime &&
                moment(detaillist?.custReturnVisitRecords?.[0]?.returnVisitTime).format(
                  'YYYY-MM-DD HH:mm:ss',
                )}
            </Descriptions.Item>
            <Descriptions.Item label="邮箱">{detaillist?.email}</Descriptions.Item>
            <Descriptions.Item label="最后回访人员">
              {detaillist?.custReturnVisitRecords?.[0]?.returnVisitPerson}
            </Descriptions.Item>
            <Descriptions.Item label="线索来源">{detaillist?.clueSource}</Descriptions.Item>
            <Descriptions.Item
              label={
                <div style={{ width: 12, height: 2, backgroundColor: 'black', margin: '0 auto' }} />}
            >
              {detaillist?.unknow}
            </Descriptions.Item>
          </Descriptions>
              ...
              <div className={Styles.descBox}>
                <div className={Styles.context}>
                  <Descriptions column={6}>
                    <Descriptions.Item label="回访人员">
                      {item?.returnVisitPerson}
                    </Descriptions.Item>
                    <Descriptions.Item label="回访状态">
                      {KEFU_VISIT[item?.status]}
                    </Descriptions.Item>
                    <Descriptions.Item label="回访方式"> {item?.returnVisitWay} </Descriptions.Item>
                    <Descriptions.Item label="回访时间" span={2}>
                      {item?.returnVisitTime &&
                        moment(item?.returnVisitTime).format('YYYY-MM-DD HH:mm')}
                    </Descriptions.Item>
                  </Descriptions>
                  <Descriptions>
